...
async fn _pay(client: ClientArc, req: LnPayRequest) -> Result<LnPayResponse, AppError> {
    debug!("Paying invoice: {:?}", req.payment_info);
    ...
    debug!("Gateway fee: {fee}, payment operation id: {operation_id}");
...
pub async fn handle_ws(state: AppState, v: Value) -> Result<Value, AppError> {
    debug!("Handling WebSocket pay request");
...
pub async fn handle_rest(
    ...
    debug!("Handling REST pay request");
